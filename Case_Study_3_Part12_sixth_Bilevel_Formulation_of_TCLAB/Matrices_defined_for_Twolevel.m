...
weighted_coefficient2_QR = 1000000; 
mpc_crystal_inner.QR=blkdiag(weighted_coefficient1_QR,weighted_coefficient2_QR);
mpc_crystal_inner.R = 50;
mpc_crystal_inner.R1=9000;

mpc_crystal_inner.OH = 2;
mpc_crystal_inner.NC = 1;

%The bounds of the temperature states
mpc_crystal_inner.Xmin = -100000*ones(size(model_crystal_inner.A,1),1);%-10000000 
mpc_crystal_inner.Xmax = 100000*ones(size(model_crystal_inner.A,1),1);%10000000

%The bounds of the inputs
mpc_crystal_inner.Umin = 0 - unominal(2);
%mpc_crystal.Umin = 28 - unominal';%lower bounds for u=Q1,Q2
mpc_crystal_inner.Umax = 100- unominal(2);    %upper bounds for u =Q1,Q2

mpc_crystal_inner.Dmin = 0- unominal(1);   %lower bounds for disturbance
mpc_crystal_inner.Dmax = 100- unominal(1);    %upper bounds for disturbance
...
weighted_coefficient1_QR = 1000000;
weighted_coefficient2_QR = 0;%10;%10;%1000
mpc_crystal_outer.QR=blkdiag(weighted_coefficient1_QR,weighted_coefficient2_QR);
mpc_crystal_outer.R = 50;
mpc_crystal_outer.R1=9000;

mpc_crystal_outer.OH = 2;
mpc_crystal_outer.NC = 1;

%The bounds of the temperature states
mpc_crystal_outer.Xmin = -100000*ones(size(model_crystal_outer.A,1),1);%-10000000 
mpc_crystal_outer.Xmax = 1000*ones(size(model_crystal_outer.A,1),1);%10000000

%The bounds of the inputs
mpc_crystal_outer.Umin = 0 - unominal(1);
%mpc_crystal.Umin = 28 - unominal';%lower bounds for u=Q1,Q2
mpc_crystal_outer.Umax = 100- unominal(1);    %upper bounds for u =Q1,Q2

mpc_crystal_outer.Dmin = 0- unominal(2);   %lower bounds for disturbance
mpc_crystal_outer.Dmax = 100- unominal(2);    %upper bounds for disturbance
%So range of manipulated variable is -30 < u < 70
